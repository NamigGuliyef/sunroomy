...
const CreateSubProdPlacementItem = ({ params }: { params: { id: string } }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit: SubmitHandler<ISubProductPlacementItem> = async (
    ...
    try {
      setLoading(true);
      await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/admin/dashboard/subproduct-placementItem`,
        formData,
        {
          headers: {
...
export default CreateSubProdPlacementItem;
