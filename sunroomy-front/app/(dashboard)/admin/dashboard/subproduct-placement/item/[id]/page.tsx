...
import { ISubProductPlacementItem } from "@/types/types";
...
const EditSubProdPlacementItem = ({ params }: { params: { id: string } }) => {
  const {
    register,
    handleSubmit,
    setValue,
    formState: { errors },
  } = useForm<ISubProductPlacementItem>();
  const [loading, setLoading] = useState(true);
  const { data: session } = useSession();
  const router = useRouter();
  const [data, setData] = useState<null | ISubProductPlacementItem>(null);
  useEffect(() => {
    const fetchPlacement = async () => {
      axios
        .get(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/admin/dashboard/subproduct-placementItem/${params.id}`,
          {
            headers: {
  ...
  const onSubmit: SubmitHandler<ISubProductPlacementItem> = async (
    formSubmitData,
  ) => {
        ...
        {loading ? (
          ...
          <PageWrapper>
            <h1 className="text-5xl">
              Create Subproduct Placement Combinations item{" "}
            </h1>
...
export default EditSubProdPlacementItem;
