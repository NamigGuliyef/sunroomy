...
const CreateSubProdCustom = () => {
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<ISubProductCustom>();
  const [loading, setLoading] = useState(false);
  const { data: session } = useSession();
  const router = useRouter();

  const onSubmit: SubmitHandler<ISubProductCustom> = async (formSubmitData) => {
    ...
    //   }
    // }
    // formData.append("subproductCustomId", params.id);
    try {
      setLoading(true);
      await axios.post(
    ...
    <>
      <div className="container mx-auto mt-6 max-w-[1280px] px-6">
        {loading ? (
          ...
          <PageWrapper>
            <h1 className="text-5xl">Create Subproduct Custom Combinations</h1>
